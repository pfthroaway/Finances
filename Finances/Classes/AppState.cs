...
namespace Finances
{
    ...
    {
        private static readonly DatabaseInteraction dbInteraction = new DatabaseInteraction();
        ...
        {
            AllAccounts = await dbInteraction.LoadAccounts();
            AllCategories = await dbInteraction.LoadCategories();
        ...
        {
            bool success = false;
            if (await dbInteraction.AddTransaction(transaction, account))
            {
        ...
        {
            bool success = false;
            if (await dbInteraction.ModifyTransaction(newTransaction, oldTransaction))
            {
        ...
        {
            bool success = false;
            if (await dbInteraction.DeleteTransaction(transaction, account))
            {
        ...
        {
            bool success = false;
            if (await dbInteraction.AddAccount(newAccount))
            {
        ...
        {
            bool success = false;
            if (await dbInteraction.DeleteAccount(account))
            {
        ...
        internal static async Task<bool> RenameAccount(Account account, string newAccountName)
        {
            ...
            if (await dbInteraction.RenameAccount(account, newAccountName))
            {
        ...
        {
            bool success = false;
            if (await dbInteraction.AddCategory(selectedCategory, newName, isMajor))
            {
                if (isMajor)
                {
        ...
        {
            bool success = false;
            if (await dbInteraction.RenameCategory(selectedCategory, newName, oldName, isMajor))
            {
                if (isMajor)
                {
        ...
        {
            bool success = false;
            if (await dbInteraction.RemoveMajorCategory(selectedCategory))
            {
                foreach (Transaction transaction in AllTransactions)
                {
        ...
        {
            bool success = false;
            if (await dbInteraction.RemoveMinorCategory(selectedCategory, minorCategory))
            {
                foreach (Transaction transaction in AllTransactions)
                {
